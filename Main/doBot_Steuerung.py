...
class SteuerungControl:
    def __init__(self, anzahl_tubes=None):
        print("Steuerung")
        self.anzahl_tubes = anzahl_tubes
        print("Anzahl Tubes: " + str(anzahl_tubes))
    ...
    def steuerung(self):
        ...
        if state == dType.DobotConnect.DobotConnect_NoError:
            ...
            for i in range(1):  # TODO für mehrere Codes differenz der tubes messen
                ...
                dType.SetPTPCmd(api, dType.PTPMode.PTPJUMPXYZMode, -66.8816 + offset_x, -221.5284 + offset_y, 65.2446,
                                -80, 1)
                ...
                dType.SetWAITCmd(api, 2000)
                # TODO aufschrauben
                dType.SetIOMultiplexingEx(api, 17, 1,
                                          0)  # wichtig zu konfiguration des pins, letzter parameter is queued 0 ist wichtig
                dType.SetIODO(api, 17, 1, 0)  # 12 Volt On
                dType.dSleep(10000)
                dType.SetIODO(api, 17, 0, 0)  # 12 Volt Off
                dType.SetPTPCmd(api, dType.PTPMode.PTPJUMPXYZMode, -86.8816 + offset_x, -221.5284 + offset_y, 65.2446,
                                -80)
