...
class TrackingLogAdapter:
    ...
    def insert_tracking_log(self,exp_id, probe_nr, Startstation, Startzeit, Zielstation, Zielzeit, Dauer, Zeitstempel):
        try:
            with self.db as conn:
                conn.execute('''
                    INSERT INTO TrackingLog (exp_id,probe_nr, Startstation, Startzeit, Zielstation, Zielzeit, Dauer, Zeitstempel)
                    VALUES (?, ?, ?, ?, ?, ?, ?,?)
                ''', (exp_id,probe_nr, Startstation, Startzeit, Zielstation, Zielzeit, Dauer, Zeitstempel))
    ...
    def get_tracking_logs_by_exp_id(self, exp_id):
        try:
            with self.db as conn:
                cursor = conn.execute('''
                    SELECT * FROM TrackingLog WHERE exp_id = ?
                ''', (exp_id,))
                tracking_logs = cursor.fetchall()
                return tracking_logs
        except Exception as e:
